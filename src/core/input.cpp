...
void init_keyboard(void) {
    ...
    keyboard_manager.keys = reinterpret_cast<Uint8 *>(&keyboard_manager.key_buffers[keyboard_manager.index]);
}

void swap_keyboard_buffers(void) {
    keyboard_manager.index = !keyboard_manager.index;

    memset(keyboard_manager.key_buffers[keyboard_manager.index], 0, sizeof(*keyboard_manager.keys) * static_cast<Uint32>(kai::Key::count));
